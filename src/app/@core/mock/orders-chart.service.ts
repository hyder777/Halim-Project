...
import {CSVRecord} from "../data/CSVModel";
// import {CSVRecord} from "../data/CSVModel";
...
export class OrdersChartService extends OrdersChartData {
  ...
  constructor(private period: PeriodsService) {
    ...
    this.data = {
      ...
      cvsFile: [],
      // dataFromCvSFile: this.getDataFromCVSFile(),
    };
  }
  ...
  private getDataForYearPeriod(): OrdersChart {
    return {
      chartLabel: this.getDataLabels(42, this.period.getYears()),
      linesData: [
        [
  ...
  getDataFromCVSFile( records: CSVRecord[]): OrdersChart {
      let listOfSumEAdEL: number[] = [];
      // let test: string[] = [];
      for (let i = 0; i < 200 ; i++) {
        // if(i%10 == 0)
        //   test.push(i);
        if(records[i].sum_ead_el !== undefined && records[i].sum_ead_el.replace(/[^\d\.\-]/g, "") !== undefined)
          listOfSumEAdEL.push(parseFloat(records[i].sum_ead_el.replace(/[^\d\.\-]/g, "")));
      }
      //this.getDataLabels(50, this.period.getWeeks())
      this.data["cvsFile"] = {
        chartLabel: [],
        linesData: [listOfSumEAdEL],
      };

     return this.data["cvsFile"];
  }
